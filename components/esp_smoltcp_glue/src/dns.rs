...
pub unsafe extern "C" fn smoltcp_resolve(h: *mut IfaceCtx,
    ...
    // is cheap. Explicit type because `None` alone can't be inferred
    // through the dns::Socket::new ManagedSlice bound.
    // DnsQuery isn't Clone, so vec![None; 4] is rejected — construct
    // the slots explicitly.
    let queries: alloc::vec::Vec<Option<dns::DnsQuery>> =
